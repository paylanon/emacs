/* ffemacs_daemon --- Start Emacs alongside daemon, or connect to running daemon.
  ...
  *p = 0;
  ...
  new_cmdline = alloca (MAX_PATH
			...
			+ ((nShow == SW_SHOWMINNOACTIVE
			   ...
			   ? (max (sizeof (iconic_opt), sizeof (maximized_opt))) + sizeof ("-c -n -a \"\" ")
			   : 0)
			+ 3);
...
#else
  strcat (new_cmdline, "\\emacsclientw.exe\" -c -n -a \"\" ");
#endif
